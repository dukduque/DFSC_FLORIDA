...
policy  = -1 # 1: %50,50%, 2: 99%,1% 3: 1%,99%
supply_factor = 1.5

zone = 3 #1:North 2: Center, 3 South
...
for s in range(N):
    ...
    if s == 0: #Scenario in which there is a demand surge in the west
        ...
        for dn in demand_nodes:
            lat_dn = fl_df['latitude'][dn]
            lon_dn = fl_df['longitude'][dn]
            fl_df.loc[dn,demand_sce] =  fl_df.loc[dn,'demand']*(s**2)
            
outcomes_prob = None
if policy == 1:
    outcomes_prob = np.array([0.5, 0.5])
elif policy == 2:
    outcomes_prob = np.array([0.99, 0.01])
elif policy == 3:
    outcomes_prob = np.array([0.01, 0.99])
else:
    outcomes_prob = None
print(outcomes_prob)
...
print(zone, len(demand_nodes), len(supply_nodes))
...
if __name__ == '__main__':
    ...
    if mode =='build_policy':
        ...
        CutSharing.options['max_stage_with_oracle'] = 2
        CutSharing.options['max_iters_oracle_ini'] = 50
        CutSharing.options['cut_selector'] = None#CutSharing.LAST_CUTS_SELECTOR#CutSharing.SLACK_BASED_CUT_SELECTOR #CutSharing.LAST_CUTS_SELECTOR
        CutSharing.options['max_cuts_last_cuts_selector'] = 5000
        
        T = len(irma_sample_path)
        alg = SDDP(T, DFSC_model_builder, DFSC_random_builder)
        alg.add_oracle_model(DFSC_oracle_model)
        alg.run(instance_name='DieselFuel-Florida - Model 2 - T%i' %(T), dynamic_sampling=False)
        sim_result = alg.simulate_policy(DFSC_random_builder())
        
        sample_path, forward_sol, models = alg.simulate_single_scenario(DFSC_random_builder(), sample_path=irma_sample_path)
        unmet_demand = [{dn:m.getVarByName('z[%s]' %(dn)).X for dn in demand_nodes} for m in models]
        policy_on_irma = {'sample_path': sample_path, 'unmet_demand':unmet_demand}
        with open('../data/irma_policy_zone%i.p' %(zone), 'wb') as fp:
    ...
    sample_path = None
    for z in [1,2,3]:
        policy_on_irma = pickle.load(open('../data/irma_policy_zone%i.p' %(z), 'rb'))
        if z == 1:
            sample_path = policy_on_irma['sample_path']
            unmet_demand = policy_on_irma['unmet_demand']
            print(unmet_demand)
        else:
            for (t, ud_t) in enumerate(unmet_demand):
                ud_t.update(policy_on_irma['unmet_demand'][t])
                sample_path[t].update(policy_on_irma['sample_path'][t])
